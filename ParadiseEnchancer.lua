...
local function hideOpenAnimation()
    ...
    activeAnimationConnection = RunService.RenderStepped:Connect(function()
...
local function updateGiftCooldowns()
    ...
    for i = 1, 9 do
        local giftId = "Gift" .. i
        local claimedGift = player.ClaimedGifts[giftId]
        ...
        if not claimedGift.Value then
            ...
            -- Si un gift est disponible maintenant, le prendre immédiatement (priorité au plus bas numéro)
...
local function rejoinServer()
    -- Utiliser Teleport au lieu de TeleportToPlaceInstance pour rejoindre un nouveau serveur
...
local function updateQuestLabels()
    -- Utiliser les données du cache au lieu de refaire les appels
    local openData = questCache.openData
    local playData = questCache.playData
    local winData = questCache.winData
...
-- BOUCLE PRINCIPALE
-- ====================================
-- Cache pour les données de quêtes (éviter les appels répétés)
local questCache = {
    openData = nil,
    playData = nil,
    winData = nil,
...
local UI_UPDATE_INTERVAL = 1 -- Mettre à jour l'UI toutes les 1 seconde
local lastBattleUICheck = 0
local BATTLE_UI_CHECK_INTERVAL = 0.5 -- Vérifier l'UI Battle toutes les 0.5 secondes

-- Fonction pour mettre à jour le cache des quêtes
local function updateQuestCache()
    questCache.openData = getQuestData("Open")
    questCache.playData = getQuestData("Play")
    questCache.winData = getQuestData("Win")
    questCache.lastUpdate = tick()
end

-- Boucle principale optimisée - s'exécute toutes les 0.1 secondes au lieu de ~60 fps
task.spawn(function()
    while task.wait(0.1) do
        local currentTime = tick()
        
        -- Mettre à jour le cache des quêtes toutes les 0.5 secondes
        if currentTime - questCache.lastUpdate >= 0.5 then
            updateQuestCache()
        end
        
        -- Garder l'UI Battle et Main/Windows activés si nécessaire (toutes les 0.5s)
        if currentTime - lastBattleUICheck >= BATTLE_UI_CHECK_INTERVAL then
            lastBattleUICheck = currentTime
            updateBattleUIState()
        end
        
        -- Auto sell items (toutes les 5 minutes)
        ...
        if State.autoRejoin and getCurrentPlayTime() >= 3000 then
            rejoinServer()
            return
        end
        ...
        if State.autoClaimGift then
            local immediateGift = updateGiftCooldowns()
            
            if immediateGift then
                if openItem(immediateGift, true) then
                    player.ClaimedGifts[immediateGift].Value = true
                    task.delay(1, updateGiftCooldowns)
                end
            elseif State.nextGiftCooldown <= os.time() and State.nextGiftId then
        ...
        -- PRIORITÉ 2: Quest Open (utiliser le cache)
        if State.autoQuestOpen and questCache.openData then
            if questCache.openData.remaining > 0 then
                openItem(questCache.openData.subject, false, math.min(5, questCache.openData.remaining), false)
            end
        end
        ...
        -- Quest Play (utiliser le cache)
        if State.autoQuestPlay and questCache.playData then
            if questCache.playData.remaining > 0 and currentTime - State.lastBattleCreateTime >= CONFIG.BATTLE_COOLDOWN then
                State.lastBattleCreateTime = currentTime
                createBattleWithBot(string.upper(questCache.playData.subject))
            end
        end
        
        -- Quest Win (utiliser le cache)
        if State.autoQuestWin and not (State.autoQuestPlay and questCache.playData) then
            if questCache.winData and questCache.winData.remaining > 0 and currentTime - State.lastBattleCreateTime >= CONFIG.BATTLE_COOLDOWN then
                State.lastBattleCreateTime = currentTime
                createBattleWithBot("CLASSIC")
            end
        end
        
        -- Mise à jour des labels UI (seulement toutes les 1 seconde)
...
-- Initialiser les cooldowns des gifts au démarrage
task.spawn(function()
